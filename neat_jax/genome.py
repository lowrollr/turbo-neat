...
class Genome:
    ...
    def condensed_node_mask(self) -> chex.Array:
        ...
        if self.is_single:
            ...
            node_mask = jnp.zeros_like(self.node_mask)
            node_mask = node_mask.at[
                jnp.arange(self.batch_size)[..., None], self.graph.to_nodes
            ].max(conn_mask)
    ...
    def get_initialized_innovation_ids(self, no_id_val=None) -> chex.Array:
        return jnp.where(
            ...
            no_id_val
            if no_id_val is not None
            else jnp.iinfo(self.graph.innovation_ids).max,
        )
    ...
    def forward(
        self, x: chex.Array, activation_selector: ActivationSelector, diff_mode=False
    ) -> chex.Array:
        ...
        * we generally expect many calls to forward w/ the same graph, so topsort is not called here
        ...
        node_values = jnp.broadcast_to(
            self.node_biases, input_shape + (self.node_biases.shape[-1],)
        )
        ...
        def propagate(index: int, node_values: chex.Array) -> chex.Array:
            ...
            activated_value = (
                enabled
                * activation_selector(
                    self.node_activation_ids[from_node], node_values[..., from_node]
                )
                * weight
            )
        ...
        if diff_mode:
            ...
            node_values = jax.lax.fori_loop(
                0, self.condensed_size, propagate, node_values
            )
        ...
        output_activations = jax.vmap(activation_selector)(
            output_activation_ids, jnp.moveaxis(output, -1, 0)
        )
    ...
    def init_connection(
        ...
        weight = (
            jax.random.normal(rng, shape=(), dtype=jnp.float32) * weight_std
            + weight_mean
        )
        return self.replace(  # pylint: disable=no-member
            graph=self.graph.replace(
                ...
                innovation_ids=self.graph.innovation_ids.at[self.next_conn_idx].set(
                    innovation_id
                ),
    ...
    def fully_connect(
        ...
        return jax.lax.fori_loop(
            ...
            condensed_size=num_connections, num_initial_connections=num_connections
        )

    def partially_connect(
        self,
        rng: chex.PRNGKey,
        innovation_id: int,
        weight_mean: float,
        ...
        to_nodes = jax.random.randint(
            node_key,
            shape=(self.input_size,),
            minval=self.input_size,
            maxval=self.input_size + self.output_size,
        )
        ...
        return jax.lax.fori_loop(
            ...
            condensed_size=self.input_size, num_initial_connections=self.input_size
        )
    ...
    def extend_capacity(self, amount: int) -> Genome:
        ...
        * this will cause JIT recompilation, so it should be called sparingly
        ...
        padded_act_ids = jnp.pad(
            self.node_activation_ids, (0, amount), constant_values=0
        )
    ...
    def topsort_and_condense(self) -> Genome:
        ...
        * assigns graph.condensed_size = number of edges in the topological order
        ...
        graph = self.graph
        init_reachable = (
            jnp.zeros(self.capacity, dtype=jnp.bool_).at[self.input_idxs].set(True)
        )
        topsort_state = TopsortState(
            reachable=init_reachable,
            cur_nodes=init_reachable,
            node_rank=jnp.full(
                self.capacity, jnp.iinfo(jnp.int32).max, dtype=jnp.int32
            ),
            cur_rank=0,
        )
        ...
        def body_fn(state: TopsortState, forward: bool):
            from_, to_ = (
                (graph.from_nodes, graph.to_nodes)
                if forward
                else (graph.to_nodes, graph.from_nodes)
            )
        ...
        init_bw_reachable = (
            jnp.zeros(self.capacity, dtype=jnp.bool_).at[self.output_idxs].set(True)
        )
        ...
        valid_edges = (
            common[graph.from_nodes] & common[graph.to_nodes] & graph.enabled_mask
        )
    ...
    def crossover(self, rng: chex.PRNGKey, other: Genome) -> Genome:
        ...
        * assumes both genomes are ordered by innovation
        ...
        # cross connections
        common_innovations = self.graph.innovation_ids == other.graph.innovation_ids
        take_conn_from_other = jax.random.bernoulli(
            rng_conn, 0.5, shape=(self.capacity,)
        )
        take_conn_from_other &= common_innovations

        # cross nodes
        take_node_from_other = jax.random.bernoulli(
            rng_node, 0.5, shape=(self.capacity,)
        )
        ...
        return self.replace(  # pylint: disable=no-member
            ...
            node_activation_ids=jnp.where(
                take_node_from_other,
                other.node_activation_ids,
                self.node_activation_ids,
            ),
            node_biases=jnp.where(
                take_node_from_other, other.node_biases, self.node_biases
            ),
        )

    def get_hidden_node_counts(self, use_condensed=True) -> chex.Array:
        valid_node_mask = (
            jnp.logical_not(self.output_mask | self.input_mask)
            & (self.condensed_node_mask if use_condensed else True)
            & self.node_mask
        )
    ...
    def get_valid_connections(self, node_id: jnp.int32) -> chex.Array:
        ...
        already_connected_to = jnp.where(
            connect_from_node_id_mask & self.graph.enabled_mask,
            self.graph.to_nodes,
            node_id,
        )
    ...
    def add_random_connection(
        ...
        return jax.lax.cond(
            ...
            lambda _: jax.lax.cond(
                is_existing_connection,
                lambda _: self.update_connection(
                    init_rng, innovation_id, weight_mean, weight_std
                ),
                lambda _: self.init_connection(
                    init_rng, from_node, to_node, innovation_id, weight_mean, weight_std
                ),
                operand=None,
            ),
            lambda _: self,
    ...
    def add_random_node(
        self,
        rng: chex.PRNGKey,
        innovation_id_1: chex.PRNGKey,
        innovation_id_2: chex.PRNGKey,
        num_activation_fns: int,
    ) -> Genome:
        ...
        return jax.lax.cond(
            mask.any(),
            lambda _: self._add_node(
                rng, mask, innovation_id_1, innovation_id_2, num_activation_fns
            ),
            lambda _: self,
            operand=None,
        )
    ...
    def _add_node(
        ...
        from_node, to_node = (
            self.graph.from_nodes[replace_idx],
            self.graph.to_nodes[replace_idx],
        )
        ...
        # add new connections
        genome = self.init_connection(
            rng_conn1, from_node, new_node_idx, innovation_id_1, 1.0, 0.0
        )
        genome = genome.init_connection(
            rng_conn2, new_node_idx, to_node, innovation_id_2, old_weight, 0.0
        )
        ...
        return genome.replace(
            ...
            node_activation_ids=genome.node_activation_ids.at[new_node_idx].set(
                activation_id
            ),
    ...
    def mutate_connections(
        self, rng: chex.PRNGKey, mutation_prob: float, noise_std: float
    ) -> Genome:
        ...
        mutate_mask = jax.random.bernoulli(
            rng_mutate, mutation_prob, shape=(self.capacity,)
        )
    ...
    def mutate_nodes(
        self,
        rng: chex.PRNGKey,
        mutate_act_prob: float,
        mutate_bias_prob: float,
        num_activation_fns: int,
        bias_noise_std: float,
        allow_output_activation_mutation: bool = False,
        allow_input_activation_mutation: bool = False,
        allow_output_bias_mutation: bool = True,
        allow_input_bias_mutation: bool = False,
    ) -> Genome:
        """Mutate the activation functions and biases of nodes."""
        rng_mutate_act, rng_mutate_bias, rng_activation, rng_bias = jax.random.split(
            rng, 4
        )
        # randomly select nodes to mutate
        act_mutate_mask = jax.random.bernoulli(
            rng_mutate_act, mutate_act_prob, shape=(self.capacity,)
        )
        ...
        bias_mutate_mask = jax.random.bernoulli(
            rng_mutate_bias, mutate_bias_prob, shape=(self.capacity,)
        )
        bias_mutate_mask &= (
            self.node_mask
        ...
        activation_ids = jax.random.choice(
            rng_activation, num_activation_fns, shape=(self.capacity,)
        )
        # randomly select new biases for the selected nodes
        bias_noise = (
            jax.random.normal(rng_bias, shape=(self.capacity,)) * bias_noise_std
        )
        # mutate the activation functions and biases of the selected nodes
        node_activation_ids = self.node_activation_ids
        node_activation_ids = jnp.where(
            act_mutate_mask, activation_ids, node_activation_ids
        )
        new_bias = jnp.where(
            bias_mutate_mask, self.node_biases + bias_noise, self.node_biases
        )
        return self.replace(  # pylint: disable=no-member
            node_activation_ids=node_activation_ids, node_biases=new_bias
        )

    def compatibility_distance(
        self, other: Genome, c_conn: float, c_weight: float
    ) -> chex.Array:
        ...
        non_homologous_counts = (
            non_homologous_conn_count_self + non_homologous_conn_count_other
        )
...
def init_genome(
    ...
    if mode == "full":
        ...
        innovation_ids = jnp.arange(
            initial_innovation_id,
            initial_innovation_id + (batch_size * num_conns),
            step=num_conns,
        )
        batched_genome = apply(
            batched_genome,
            Genome.fully_connect,
            batch_rng,
            innovation_ids,
            weight_mean=weight_mean,
            weight_std=weight_std,
